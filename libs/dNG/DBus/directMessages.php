...
:#\n*/
		...
		while ((is_bool ($return))&&($return)&&($continue_check)&&($timeout_time > /*#ifndef(PHP4) */(microtime (true))/* #*//*#ifdef(PHP4):(time ()):#*/))
		{
			...
			if (is_object ($message))
			{
				...
				if (is_string ($le)) { $continue_check = true; }
			}

			if ($continue_check) { $return = $this->callback ($le,$message); }

			if ($messages > -1)
			{
				$messages--;
				if ($messages == 0) { $continue_check = false; }
			}
		}
...
*/
	/*#ifndef(PHP4) */public /* #*/function &read ($timeout)
	{
		...
		while ((is_bool ($return))&&($return)&&(($continue_check)||($this->dbus_broken_length))&&($timeout_time > (time ())))
		{
			$continue_check = false;
			...
			else
			{
				...
				{
					if (strlen ($data_read) == 16) { $continue_check = true; }
					else
					{
						$this->dbus_broken_data_read = $data_read;
...
*/
	/*#ifndef(PHP4) */protected /* #*/function sendBuildMessage ($type,$path,$interface,$member,$destination = "",$flags = NULL,$signature = "",$parameter = NULL)
	{
		...
		if (($return)&&(is_string ($path))&&(strlen ($path))&&(is_string ($interface))&&(is_string ($member))&&(strlen ($member))&&(is_string ($destination)))
		{
			...
			{
				if ($parameter == NULL) { $continue_check = false; }
				$body_exists = true;
			}
...
array (3,(array ("s",$member))),
			...
			if (($continue_check)&&($body_exists))
			{
				$body_raw = $return->marshalArray ($signature,$parameter);

				if (is_bool ($body_raw)) { $continue_check = false; }
				else
				{
			...
			if ($continue_check)
			{
				...
				if (is_bool ($header_raw)) { $continue_check = false; }
				else
				{
			...
			if ($continue_check) { $return->set ($header_array,$header_raw); }
			else { $return = false; }
		}
...
/**
	...
	* 
	* @param  directMessage &$message directMessage object
	* @return mixed Array (message -> directMessage object,body -> body array) on
...
*/
	/*#ifndef(PHP4) */protected /* #*/function sendSyncResponse (&$message)
	{
		...
		while ((is_bool ($return))&&($return)&&($continue_check)&&($timeout_time > (time ())))
		{
			$continue_check = false;
			...
			if ($continue_check)
			{
				...
				if (($header_array)&&($header_array[1] == $serial)) { $continue_check = false; }

				if ($continue_check) { $this->callback ($le,$response); }
				else
				{
					...
					if ($return)
					{
						...
						if (($this->dbus_name)&&($header_array))
						{
							if ($header_array[1] == $this->dbus_name) { $continue_check = true; }
						}
						else { $continue_check = true; }
					}
					else { $continue_check = true; }
